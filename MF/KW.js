...
async function getRecommendSheetTags() {
    const res = (await axios_1.default.get(`http://wapi.kuwo.cn/api/pc/classify/playlist/getTagList?cmd=rcm_keyword_playlist`)).data.data;
    const data = res
        .map((group) => ({
        title: group.name,
...
async function getRecommendSheetsByTag(tag, page) {
    ...
    if (tag.id) {
        if (tag.digest === "10000") {
            res = (await axios_1.default.get(`http://wapi.kuwo.cn/api/pc/classify/playlist/getTagPlayList?loginUid=0&loginSid=0&appUid=76039576&pn=${page}&id=${tag.id}&rn=${pageSize}`)).data.data;
        } else {
		...
		res = (await axios_1.default.get(`https://wapi.kuwo.cn/api/pc/classify/playlist/getRcmPlayList?loginUid=0&loginSid=0&appUid=76039576&pn=${page}&rn=${pageSize}&order=hot`)).data.data;
    }
...
async function getMediaSource(musicItem, quality) {
	// 无视任何音乐,全部默认128k
	const res = (await axios_1.default.get(`http://106.52.249.141:13006/api?action=kuwo&id=${musicItem.id}&format=${quality}`)).data;
	return {
		url: res.url,
	};
}
...
async function getMusicInfo(musicItem) {
	// 获取专辑图片
    const infoimg = (await axios_1.default.get(`http://artistpicserver.kuwo.cn/pic.web?type=rid_pic&pictype=url&size=256&rid=${musicItem.id}`)).data;
	return {
		artwork: infoimg,
	};
}
