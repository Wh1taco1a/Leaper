...
function formatMusicItem(_) {
    ...
    return {
        id: _.id,
        artwork: album && _.picUrl && album.picUrl,
        title: _.name,
        artist: (_.ar || _.artists)[0].name,
        album: album && _.name && album.name,
...
async function getValidMusicItems(trackIds) {
    ...
    try {
        ...
        const validTrackIds = res.data.map((_) => _.id);
...
async function getMediaSource(musicItem, quality) {
    const res = (await axios_1.default.get(`http://106.52.249.141:13006/api?action=wyy&id=${musicItem.id}&format=${quality}`)).data;
    return {
        url: res.url,
    };
}
