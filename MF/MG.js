...
async function getLyricbak(musicItem) {
    const headers = {
        Accept: "application/json, text/javascript, */*; q=0.01",
        "Accept-Encoding": "gzip, deflate, br",
        "Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
...
async function getMediaSource(musicItem, quality) {
	const resource = (await (0, axios_1.default)({
		url: `https://app.c.nf.migu.cn/MIGUM2.0/strategy/listen-url/v2.4?netType=01&resourceType=2&songId=${musicItem.id}&toneFlag=PQ&lowerQualityContentId=${musicItem.copyrightId}`,
		headers: {
			"Accept": "*/*",
			"Accept-Language": "zh-CN,zh;q=0.9,en;q=0.8,en-GB;q=0.7,en-US;q=0.6",
	...
	return {
		url: resource.url,
		artwork: fixImageUrl(resource.song.img1) || musicItem.artwork,
		lrc: resource.lrcUrl,
	};
}

function fixImageUrl(imageUrl) {
  const validUrlPattern = /^https?:\/\//i; // 正则用于检查是否为有效的 HTTP 连接
  if (!validUrlPattern.test(imageUrl)) {
    // 如果不是有效的 HTTP 连接，添加前缀
    imageUrl = "https://d.musicapp.migu.cn" + imageUrl;
  }
  return imageUrl;
}
...
module.exports = {
    ...
    srcUrl: "http://www.skeo.net/music/migu.js",
